const { addFollowupCSV,addFollowupBaramatiCSV } = require('../services/sftpService');
...
const addFollowupControllerBaramati = async (req, res) => {
    const { leadID, followup } = req.body;
console.log("------------ id ----------")
    console.log(leadID);
    console.log("followup");
    ...
    try {
       const data = await addFollowupBaramatiCSV(leadID, followup);
        return res.json({ success: true, lead: data });
        console.log(data);
      } catch (err) {
        console.error('Error updating leads:', err.message);
  ...
  module.exports = { addFollowupController,addFollowupControllerBaramati };
