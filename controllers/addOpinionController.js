const { addGothaFollowupCSV ,addGothaFollowupCSVBaramati} = require('../services/sftpService');
...
const addOpinionControllerBaramati = async (req, res) => {
  const { leadID, followup } = req.body;
  console.log("------------ id ----------");
  console.log(leadID);
  console.log(followup);
  ...
  try {
    const data = await addGothaFollowupCSVBaramati(leadID, followup);
    console.log("Saved followup:", data); // log BEFORE sending response
  ...
  module.exports = { addOpinionController,addOpinionControllerBaramati };
