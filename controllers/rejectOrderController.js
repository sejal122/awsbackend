...
const { RejectOrderAndUploadFile,RejectOrderAndUploadFileShrirampur,RejectOrderAndUploadFileBaramati} = require('../services/sftpService');
...
try {
 ...
 const  rejectOrderBaramati=async (req,res)=> {
...
try {
    const data = await RejectOrderAndUploadFileBaramati(doc_number,approvedHistoryFormat);
   res.status(200).json(data);
...
module.exports = { rejectOrder ,rejectOrderShrirampur,rejectOrderBaramati};
